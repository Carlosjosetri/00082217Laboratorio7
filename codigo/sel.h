...
Matrix createLocalK(int e,mesh &m){
    ...
    float u_bar,nu,rho,Ae,J,D,tao,kappa,nabla,lamda;
    
    //Componentes de K
    // [ A+K  J+G ]
    // [  D   0 ]
    Matrix matrixA,matrixK,matrixG,matrixD,matrixJ;
    ...
    productRealMatrix(tao*J/(6*D),productMatrixMatrix(g_matrix,productMatrixMatrix(Alpha,Beta,2,2,6),6,2,6),matrixA);
    ...
    //Preparando matrixJ (Creada por Trigueros nombrada como J)(lo cual no fue buena idea ):(
    nabla =m.getParameter(NABLA);
    calculateBPrima(BPrima);
    productRealMatrix(nabla*Ae/(D*D),productMatrixMatrix(Betat,productMatrixMatrix(Alphat,productMatrixMatrix(Alpha,BPrima,2,2,6),2,2,6),6,2,6),matrixJ);

    //Preparando matrixG (En clase conocida simplemente como G)
    rho = m.getParameter(DENSITY);
    lamda =m.getParameter(LAMDA);
    productRealMatrix((lamda*J)/(6*D),productMatrixMatrix(g_matrix,productMatrixMatrix(Alpha,BPrima,2,2,3),6,2,3),matrixG);
    ...
    ubicarSubMatriz(K,0,5,6,8,sumMatrix(matrixJ,matrixG,6,3));
