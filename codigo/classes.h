...
enum parameters2 {TAO,KAPPA,NABLA,LAMDA,NOIR};
...
class mesh{
    ...
    public:
        void setParameters(float tao,float kappa, float nabla, float lamda, float x , float y,float nir){
            parameters[TAO]=(tao/4);
            parameters[KAPPA]=kappa;
            parameters[NABLA]=(nabla/8);
            parameters[LAMDA]=(lamda*(2/3));
            parameters[EXTERNAL_FORCE_X]=x;
            parameters[EXTERNAL_FORCE_Y]=y;
            parameters[NOIR]=nir;
